...
public class SimulationResults {
	...
	public SimulationResults(int days, List<Service> servicesList) {
		this.totalDays = days;
	...
	public Result addResult(int minute, double powerProduction, double electricalLoad, double batteryUsageReal, BasicBattery battery, double lastGlobalReward, double globalReward) {		
		this.resultados[minute] = new Result(minute, powerProduction, electricalLoad, batteryUsageReal, battery, servicesList, lastGlobalReward, globalReward);
		return this.resultados[minute];
	}
	
	public Result addResult(int minute, double powerProduction, double electricalLoad, double batteryUsageReal, BasicBatteryWithGenerator battery, double lastGlobalReward, double globalReward) {		
		this.resultados[minute] = new Result(minute, powerProduction, electricalLoad, batteryUsageReal, battery, servicesList, lastGlobalReward, globalReward);
		return this.resultados[minute];
	}
	...
	public void showResults(boolean saveGraph) {
		...
		double[] yDataBatteryunderCapacityLoss = new double[totalMinutes];
		double[] yDataBatteryunderCapacityLossSum = new double[totalMinutes];
		double yDataBatteryunderCapacityLossSumTmp = 0;
		double[] yDataEnergyUsage = new double[totalMinutes];
		double[] yDataGlobalReward = new double[totalMinutes];
		double[] yDataLastGlobalReward = new double[totalMinutes];

		int servicesNum = servicesList.size();
		double[][] yDataServicesUsage = new double[servicesNum][totalMinutes];
		double[][] yDataServicesReward = new double[servicesNum][totalMinutes];
		...
		for(int i = 0; i < this.totalMinutes; i++) {
			...
			yDataEnergyUsage[i] = yDataPowerProduction[i] - yDataElectricalLoad[i];
			...
			yDataBatteryunderCapacityLoss[i] = -this.resultados[i].getBatteryUnderCapacityLoss() * 60;
			yDataBatteryunderCapacityLossSumTmp += this.resultados[i].getBatteryUnderCapacityLoss();
			yDataBatteryunderCapacityLossSum[i] = yDataBatteryunderCapacityLossSumTmp;

			yDataGlobalReward[i] = this.resultados[i].getGlobalReward();
			yDataLastGlobalReward[i] = this.resultados[i].getLastGlobalReward();
			...
			for(int j = 0; j < servicesNum; j++) {
				...
				yDataServicesReward[j][i] = this.resultados[i].getServiceReward()[j];
			}
		...
		// Create Chart
		/*XYChart chartPowerProduction = QuickChart.getChart("Solar Power", "X", "Y", "Wh", xData, yDataPowerProduction);
		XYChart chartElectricalLoad = QuickChart.getChart("Electrical Load", "X", "Y", "Wh", xData, yDataElectricalLoad);
		XYChart chartElectricalUsage = QuickChart.getChart("Electrical Charge", "X", "Y", "Wh", xData, yDataEnergyUsage);
		XYChart chartBatteryPercentage = QuickChart.getChart("Battery Percentage (permille)", "X", "Y", "‰", xData, yDataBatteryPercentage);
		XYChart chartBatteryUsage = QuickChart.getChart("Battery Usage", "X", "Y", "Wh", xData, yDataBatteryUsage);
		XYChart chartBatteryLoss = QuickChart.getChart("Battery Loss", "X", "Y", "Wh", xData, yDataBatteryunderCapacityLoss);*/
		XYChart chartBatteryLossSum = QuickChart.getChart("Battery Loss Sum", "X", "Y", "Wh", xData, yDataBatteryunderCapacityLossSum);
		XYChart chartGlobalReward = QuickChart.getChart("Global Reward", "X", "Y", "R", xData, yDataGlobalReward);
		
		// Show them
		/*new SwingWrapper(chartPowerProduction).displayChart();
		new SwingWrapper(chartElectricalLoad).displayChart();
		new SwingWrapper(chartElectricalUsage).displayChart();
		
		new SwingWrapper(chartBatteryPercentage).displayChart();
		new SwingWrapper(chartBatteryUsage).displayChart();*/
		new SwingWrapper(chartBatteryLossSum).displayChart();
		//new SwingWrapper(chartGlobalReward).displayChart();
		...
		chartEnergy.addSeries("Electrical Production (Wh)", xData, yDataEnergyUsage).setMarker(SeriesMarkers.NONE);
		chartEnergy.addSeries("Solar Power (Wh)", xData, yDataPowerProduction).setMarker(SeriesMarkers.NONE);
		chartEnergy.addSeries("Battery Level (‰)", xData, yDataBatteryPercentage).setMarker(SeriesMarkers.NONE);
		chartEnergy.addSeries("Battery Usage (Wh)", xData, yDataBatteryUsage).setMarker(SeriesMarkers.NONE);
		chartEnergy.addSeries("Battery Loss (Wh)", xData, yDataBatteryunderCapacityLoss).setMarker(SeriesMarkers.NONE);
		...
		if(saveGraph) {			
			try {
				...
				BitmapEncoder.saveBitmapWithDPI(chartServicesReward, this.outputFolder+"chartServicesReward.png", BitmapFormat.PNG, 100);
				BitmapEncoder.saveBitmapWithDPI(chartBatteryLossSum, this.outputFolder+"chartBatteryLossSum.png", BitmapFormat.PNG, 100);
			} catch (IOException e) {
	...
	public void printStats() {
		...
		System.out.println("Generator: " + batteryunderCapacityLossSumTmp);
